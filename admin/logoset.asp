...
sql = "select * from logo where lang='"&lang&"' and skins='"&skins&"'  "
...
rs.update
	...
	call addlog("设置LOGO成功!")
  response.Write "<script language=javascript>alert(""设置成功"");window.location.href=""logoset.asp""</script>"
    response.End
End If
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">LOGO设置  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<td>LOGO图片：</td>
			    <td colspan="2">
				   <input name="logourl"  type="TEXT" id="weepic"  value="<%=rs("logourl")%>" size="50"  /> 
				  <br />
				   <br />
			      <a class='sfile' href="Upload_All.Asp?add=one" title="支持上传格式(*.swf;*.gif;*.bmp)"><div style="width:100px;padding:10px; background:#28b576;color:#ffffff" align="center">网站LOGO上传</div></a>
		        <div id="mysmaimg">
				<%if x=1 then%>
...
<td colspan="2">
          ...
          <input name="logowidth"type="text" class="inputtext" id="logowidth"  value="<%=rs("logowidth")%>" size="6" maxlength="30" />
                    logo宽×高
        <input name="logoheight"type="text" class="inputtext" id="logoheight"  value="<%=rs("logoheight")%>" size="6" maxlength="30" /></td>
    </tr>
