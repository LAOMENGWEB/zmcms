...
if request("Submit")="修改" then
	...
	if ID<>"" THEN
	sql="select * from alclass where id="&ID
	END IF
	if ID1<>"" THEN
	sql="select * from alclass where id="&ID1
	END IF
	if ID2<>"" THEN
	sql="select * from alclass where id="&ID2
	END IF
...
call htmlal
	...
	call addlog("修改"&tupian&"分类")
	response.Write "<script language=javascript>alert(""类别修改成功"");window.location.href=""fl_al.asp""</script>"
    response.End
end if
%>
...
Set rs = Server.CreateObject("ADODB.Recordset")
if ID<>"" THEN
	sql="select * from alclass where id="&ID&" and lang='"&lang&"'"
	END IF
	if ID1<>"" THEN
	sql="select * from alclass where id="&ID1&" and lang='"&lang&"'"
	END IF
	if ID2<>"" THEN
	sql="select * from alclass where id="&ID2&" and lang='"&lang&"'"
	END IF
rs.open sql,conn,1,1
If rs.EOF Then
    response.Write "<script language=javascript>window.location.href=""fl_al.asp""</script>"
    response.End
End if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">修改<%=tupian%>分类  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="anlie.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="fl_al.asp" target="main"><%=tupian%>分类管理</a></div></td>
  ...
  <table width="99%"  border="0" cellpadding="0" cellspacing="0" align="center" class="dtable4">
     <tr  >
		  ...
		  rsc.open "select * from alclass where id="& request.QueryString("id")&"",conn,1,1
          response.Write("顶级分类")
          rsc.close
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from alclass where ID="& request.QueryString("id1")&"",conn,1,1
          ParentClassID=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from alclass where id="& ParentClassID&"",conn,1,1
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from alclass where ID="& request.QueryString("id2")&"",conn,1,1
          ParentClassID1=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from alclass where id="& ParentClassID1&"",conn,1,1
          response.Write("" & rsc("classname") & "")
          rsc.close
		set rsc=nothing
