...
if request("Submit")="提交" then
	...
	sql="select * from alclass Where mulu= '" & mulu & "'"
...
rs("mulu")=mulu
	...
	ChildCount=conn.execute("select count(*) from [alclass] where ParentClassID="&id)(0)
conn.execute("update [alclass] set c_child = "& checknum(ChildCount,0) &" where id="& id &"")
...
call htmlal
	...
	call addlog(""&tupian&"分类")
	response.Write "<script language=javascript>alert(""分类添加成功"");window.location.href=""fl_al.asp""</script>"
    response.End
end if
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=tupian%>分类  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
