...
if request("Submit")="修改" then
	...
	if ID<>"" then
sql="select * from xzclass where id = " & ID
end if
if id1<>"" then
sql="select * from xzclass where id = " & ID1
end if
if id2<>"" then
sql="select * from xzclass where id = " & ID2
end if
...
call htmll("","","default"&lang&".html","default.asp","m=",mde,"language=",lang,"","","","","","","","")
	...
	call addlog("修改"&xiazai&"分类")
	response.Write "<script language=javascript>alert(""类别修改成功"");window.location.href=""fl_xz.asp""</script>"
    response.End
end if
%>
...
Set rs = Server.CreateObject("ADODB.Recordset")
if ID<>"" then
sql="select * from xzclass where id = " & ID&" and lang='"&lang&"'"
end if
if id1<>"" then
sql="select * from xzclass where id = " & ID1&" and lang='"&lang&"'"
end if
if id2<>"" then
sql="select * from xzclass where id = " & ID2&" and lang='"&lang&"'"
end if
rs.open sql,conn,1,1
If rs.EOF Then
    response.Write "<script language=javascript>window.location.href=""fl_xz.asp""</script>"
    response.End
End if
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">修改<%=xiazai%>分类 <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="xiazai.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="fl_xz.asp" target="main"><%=xiazai%>分类管理</a></div></td>
  ...
  <table width="99%"  border="0" cellpadding="0" cellspacing="0" align="center" class="dtable4">
    ...
    <tr  >
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from xzclass where ID="& request.QueryString("id1")&"",conn,1,1
          ParentClassID=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from xzclass where id="& ParentClassID&"",conn,1,1
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from xzclass where ID="& request.QueryString("id2")&"",conn,1,1
          ParentClassID1=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from xzclass where id="& ParentClassID1&"",conn,1,1
          response.Write("" & rsc("classname") & "")
          rsc.close
		set rsc=nothing
