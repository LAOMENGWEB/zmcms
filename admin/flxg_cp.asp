...
if request("Submit")="修改" then
	...
	if ID<>"" THEN
	sql="select * from cpclass where id="&ID
	END IF
	if ID1<>"" THEN
	sql="select * from cpclass where id="&ID1
	END IF
	if ID2<>"" THEN
	sql="select * from cpclass where id="&ID2
	END IF
...
call htmlpro
	...
	response.Write "<script language=javascript>alert(""类别修改成功"");window.location.href=""fl_cp.asp""</script>"
    response.End
end if
%>
...
Set rs = Server.CreateObject("ADODB.Recordset")
if ID<>"" then
sql="select * from cpclass where id = " & ID&" and lang='"&lang&"'"
end if
if id1<>"" then
sql="select * from cpclass where id = " & ID1&" and lang='"&lang&"'"
end if
if id2<>"" then
sql="select * from cpclass where id = " & ID2&" and lang='"&lang&"'"
end if
rs.open sql,conn,1,1
If rs.EOF Then
    response.Write "<script language=javascript>window.location.href=""fl_cp.asp""</script>"
    response.End
End if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">修改<%=chanpin%>分类  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
  ...
  <table width="99%"  border="0" cellpadding="0" cellspacing="0"  align="center" class="dtable4">
    <tr  >
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from cpclass where ID="& request.QueryString("id1")&"",conn,1,1
          ParentClassID=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from cpclass where id="& ParentClassID&"",conn,1,1
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from cpclass where ID="& request.QueryString("id2")&"",conn,1,1
          ParentClassID1=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from cpclass where id="& ParentClassID1&"",conn,1,1
          response.Write("" & rsc("classname") & "")
          rsc.close
		set rsc=nothing
    ...
    <tr>
      <td>类别名称:</td>
     <td> <input name="classname" type="text" id="classname" value="<%=rs("classname")%>" datatype="*" nullmsg="类别名称不能为空"/>
