...
'获取添加新闻的ID  生成html开始
set rs=conn.execute("select top 1 id,zmone,zmtwo,zmthree from [news] order by id desc")
newid=rs(0)
pone=rs(1)
ptwo=rs(2)
pthree=rs(3)
...
call htmll("/html/","",""&NewName&""&Separated&""&pone&""&Separated&""&newid&""&Separated&"1.html",""&showinfo&"/index.asp","pone=",pone,"newsID=",newid,"m=",m,"language=",lang,"page=",1,"","")
else
arrContent=split(content,"[zmcmsfy]")
pages=Ubound(arrContent)+1
for i=1 to pages
call htmll("/html/","",""&NewName&""&Separated&""&pone&""&Separated&""&newid&""&Separated&""&i&".html",""&showinfo&"/index.asp","pone=",pone,"newsID=",newid,"m=",m,"language=",lang,"page=",i,"","")
next
end If
...
call addlog("添加"&xinwen&"")

response.Write "<script language=javascript>alert(""恭喜您添加成功"");window.location.href=""news_manage.asp""</script>"
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=xinwen%>  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<tr > 
<td width="10%"   ><%=xinwen%>标题：</td>

<td width="88%">
  <input  name="Title" type="text" id="Title"  size="50"  datatype="*" nullmsg="标题不能为空"/> <font color="#FF0000">*</font></td>
  </tr>
  <tr > 
<td   >标题颜色：</td>
<td>
 ...
 <tr >
   <td >Tags标签:</td>
   <td ><input name="KeyWord" type="text" id="KeyWord" size="40"  datatype="*" nullmsg="Tags不能为空"/> <font color="#FF0000">*</font> 【<a href="#" id="KeyLinkByTitle" style="color:green"> 根据标题自动获取</a>】(只对中文内容生效,多个tags之间用|隔开)</br>
   </td>
...
<select ID="ClassID" name="ClassID" datatype="*" nullmsg="所属类别不能为空">
	   <%call mxlb("NewsClass",ClassId)%>
	  </select> <font color="#FF0000">*</font><font color=red>（有三级栏目的必须选择三级栏目）</font></td>
</tr>
 <tr >
<td   ><%=xinwen%>关键词:</td>
<td>
<input name="newkey" type="text" id="newkey" size="100" /></td>
</tr>
<tr >
<td   ><%=xinwen%>描述:</td>
<td>
<input name="newms" type="text" id="newms" size="100" /></td>
</tr>
 <tr >
<td   >外链<%=xinwen%>:</td>
<td>
<input name="wlurl" type="text" id="wlurl" />
...
<td>
 ...
 afterBlur: function(){this.sync();}
		   ...
		   <tr > 
			...
			<td>
            <textarea name="bfnr" id="bfnr" style="width:700px;height:300px;visibility:hidden;"></textarea></td>
          </tr>
 ...
 afterBlur: function(){this.sync();}
 	...
 	K('input[name=appendHtml]').click(function(e) {
          ...
          <tr > 
			...
			<td>
           <textarea name="Content" id="Content" style="width:700px;height:300px;visibility:hidden;" datatype="*" nullmsg="<%=xinwen%>内容不能为空"></textarea></td>
          </tr>
 ...
 <div id="J_imageView">  </div></td>
	...
	<input name="zdfy" type="hidden" id="zdfy"  value="0" size="10">
		   ...
		   <tr >
		     <td  >属性：</td>
			 <td>
	         <input name="tj" type="checkbox" id="tj" value="1" class="none">
	         <%=xinwen%>幻灯
			 <input name="tt" type="checkbox" id="tt" value="1" class="none">置顶
			 <input name="sytj" type="checkbox" id="sytj" value="1" class="none">推荐
			 <input name="gg" type="checkbox" id="gg" value="1" class="none"> 头条	
			 
			 		 </td>
	      </tr>
		   <tr > 
            <td  >点击次数：</td>
              <td><input  name="hits" type="text" class="input" value="0" size="30"> </td>
          </tr>
          <tr > 
            <td  >发布人：</td>
              <td><input  name="user" type="text" class="input" value="<%=session("userName")%>" size="30"> </td>
          </tr>
          ...
          <tr > 
            <td   >发布时间：</td>
             <td>   <input  name="adddate" type="text" id="adddate" value="<%=now()%>" onClick="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm:ss'})" >
          提示：【可以定时发布<%=xinwen%>了，点击边框设定时间】</td>
          </tr>
          <tr > 
