...
if request("Submit")="修改" then
	...
	if id1<>"" then
	sql="select * from newsclass where id="&ID1
	end if
	if id2<>"" then
	sql="select * from newsclass where id="&ID2
	end if
	...
	call addlog("修改"&xinwen&"分类")
	response.Write "<script language=javascript>alert(""类别修改成功"");window.location.href=""fl_news.asp""</script>"
    response.End
end if
%>
...
Set rs = Server.CreateObject("ADODB.Recordset")
if ID<>"" then
sql="select * from newsclass where id = " & ID&" and lang='"&lang&"'"
end if
if id1<>"" then
sql="select * from newsclass where id = " & ID1&" and lang='"&lang&"'"
end if
if id2<>"" then
sql="select * from newsclass where id = " & ID2&" and lang='"&lang&"'"
end if
rs.open sql,conn,1,1
If rs.EOF Then
    response.Write "<script language=javascript>window.location.href=""fl_news.asp""</script>"
    response.End
End if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center"><%=xinwen%>分类修改  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="xinwen.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="fl_news.asp" target="main"><%=xinwen%>分类管理</a></div></td>
 

 </tr>
</table>
  <table width="98%"  border="0" cellpadding="0" cellspacing="0" class="dtable4" align="center">
    <tr  >
				    <td height="25" >上级分类: </td>
					<td>
			<% if request.QueryString("id")<>"" then%>
            <%
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from newsclass where id="& request.QueryString("id")&"",conn,1,1
          response.Write("顶级分类")
          rsc.close
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from newsclass where ID="& request.QueryString("id1")&"",conn,1,1
          ParentClassID=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from newsclass where id="& ParentClassID&"",conn,1,1
		...
		set rsc=nothing
		  ...
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from newsclass where ID="& request.QueryString("id2")&"",conn,1,1
          ParentClassID1=rsc("ParentClassID")
          rsc.close
		set rsc=nothing
		 
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from newsclass where id="& ParentClassID1&"",conn,1,1
          response.Write("" & rsc("classname") & "")
          rsc.close
		set rsc=nothing
