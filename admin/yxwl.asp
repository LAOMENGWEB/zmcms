...
if Request("id")="" then
Response.Write "<script>alert('错误!请选择要操作的记录!');window.location.href='yxwl.asp';</script>" 
response.end()
end if
dim id
ID=trim(request("ID"))
...
rs.open "select * from network where ID in ("&id1(i) &")",conn,0,1
dim picUrl
dim picUrlArray
dim x,y
...
next
conn.Execute ( "delete from network where id in ("&Request("id")&")" )
call addlog("删除营销网络")
response.Write "<script language=javascript>alert(""删除成功！"");window.location.href=""yxwl.asp""</script>"
end if
%>
...
function ConfirmDel()
{
   if(confirm("确定要删除选中的营销点吗？一旦删除将不能恢复！"))
     return true;
   else
     return false;
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">营销网络管理 <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="yxwladd.asp" target="main">添加营销网络</a></div></td>

 </tr>
</table>

<%
DB_Sql = "select * from network where lang='"&lang&"' "
...
Obj_Page.Setpage_Pathurl = "yxwl.asp" '执行分页的页面
...
%>
              ...
              <table width="99%" align="center" cellpadding="0" cellspacing="0"  class="dtable2">
				...
				<td width="9%" > 
                  ...
                  <td width="16%"  > 
                    <div align="center">销售点</div></td>
                  <td width="18%"  ><div align="center">地址</div></td>
                  <td width="18%" ><div align="center">联系人</div></td>
                  
                  <td width="16%" ><div align="center">创建时间</div></td>
				  <td width="16%"  style="border-right:none"><div align="center">操作</div></td>
                </tr>
<%
'/循环开始
...
%>
                ...
                <tr  >
				 <td><div align="center">
                  ...
                  <td height="40"  > 
                    <div align="center"><font color="#FF0000">[<%=rs("province")%>]</font> &nbsp; <%=rs("company")%></div></td>
                  <td ><div align="center"><%=rs("address")%></div></td>
                  <td ><div align="center"><%=rs("user")%></div></td>
                  
                  <td ><div align="center"><%=rs("adddate")%></div></td>
				  <td  style="border-right:none">
				    <div align="center"><a href="yxwlxg.asp?ID=<%=rs("ID")%>">修改</a>&nbsp; 
			        &nbsp;<a href="yxwldel.asp?ID=<%=rs("ID")%>" onClick="return confirm('确定要删除该营销网络吗？');" >删除</a></div></td>
                </tr>
 <%
rs.MoveNext
Next	
%>    
 <tr  >
                  ...
                  <td  colspan="6"  style="padding:5px; text-align:left;" ><input class="bnt" type="submit" name="button" id="button" value="批量删除"   onClick="return confirm('确定要批量删除选中的营销点吗，将不可恢复?');"/></td>
                </tr> 
  </table>
	...
	%>
		...
		<table width="99%"  cellpadding="0" cellspacing="0" class="table1" style="margin-top:10px">
          ...
          <tr   >
            <td  height="30"  style="padding-left:10px; text-align:center">  <%Response.Write "目前暂无任何信息"
Response.end%> </td>       
  </table>
