...
if request("Submit")="提交" then
	...
	sql="select * from spclass Where mulu= '" & mulu & "'"
	...
	ChildCount=conn.execute("select count(*) from [spclass] where ParentClassID="&id)(0)
conn.execute("update [spclass] set c_child = "& checknum(ChildCount,0) &" where id="& id &"")
...
call htmltv
   call htmll("","","default"&lang&".html","default.asp","m=",mde,"language=",lang,"","","","","","","","")
	...
	call addlog("添加"&shipin&"分类")
	response.Write "<script language=javascript>alert(""分类添加成功"");window.location.href=""fl_sp.asp""</script>"
    response.End
end if
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=shipin%>分类  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
