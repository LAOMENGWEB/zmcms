...
sql1="select * from yqljclass where id="&request.QueryString("id")&""
...
rs.close
set rs=nothing
call addlog("设置链接栏目排序")
end if
Set rs = server.CreateObject("adodb.recordset")
sql2 = "select * from yqljclass where id = "&request.QueryString("id")&""
...
<form id="form1" name="form1" method="post" action="xgyqljflpx.asp?id=<%=rs("id")%>">
  <label>
