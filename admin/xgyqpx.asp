...
sql1="select * from yqlj where id="&request.QueryString("id")&""
...
rs.close
set rs=nothing
call addlog("设置友情链接排序")
end if
Set rs = server.CreateObject("adodb.recordset")
sql2 = "select * from yqlj where id = "&request.QueryString("id")&""
...
<form id="form1" name="form1" method="post" action="xgyqpx.asp?id=<%=rs("id")%>">
  <label>
