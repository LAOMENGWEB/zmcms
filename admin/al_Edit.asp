...
Set rs = server.CreateObject("adodb.recordset")
sql="Select * From al Where id = "&request.QueryString("id")&""
...
'获取添加新闻的ID  生成html开始
set rs=conn.execute("select id,zmone,zmtwo,zmthree from [al] where id="&id&"")
alid=rs(0)
pone=rs(1)
ptwo=rs(2)
pthree=rs(3)
...
call addlog("修改"&tupian&"")
response.Write "<script language=javascript>alert(""恭喜您修改成功"");window.location.href=""al_gl.asp""</script>"
end if
Set rs = server.CreateObject("adodb.recordset")
sql = "select * from al where id = "&request.QueryString("id")&" and lang='"&lang&"'"
rs.Open sql, conn, 1, 1
If rs.EOF Then
    response.Write "<script language=javascript>window.location.href=""al_gl.asp""</script>"
    response.End
End If
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">修改<%=tupian%> <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<tr>
               <td width="12%" ><%=tupian%>标题：</td>
			   <td width="88%">
      <input name="Title" type="text" id="Title" value="<%=rs("title")%>" size="40" datatype="*" nullmsg="标题不能为空"/></td>
</tr>
<tr>
               <td >标题颜色：</td>
...
<tr>
  <td ><%=tupian%>关键词：</td>
    ...
    <input name="alkey" type="text" id="alkey" value="<%=rs("alkey")%>" size="100" />
    </td>
</tr>
<tr>
  <td ><%=tupian%>描述：</td>
    ...
    <input name="alms" type="text" id="alms" value="<%=rs("alms")%>" size="100" />
  </td>
	</tr>
							<%'自定义字段%>
...
<tr>
               <td>是否推荐:</td>
	  <td ><input name="tj" type="checkbox" id="tj" value="1" <% if rs("tj")=1 then response.Write("checked") end if%> /></td>
             </tr>
		    <tr > 
            <td  >点击次数：</td>
			<td >
            <input name="hits" type="text" id="hits"   value="<%=rs("hits")%>"></td>
          </tr>
			 <tr  >
      <td  >发布人：</td>
	  <td >
