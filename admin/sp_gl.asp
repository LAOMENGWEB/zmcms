...
if Request("id")="" then
Response.Write "<script>alert('错误!请选择要操作的记录!');window.location.href='sp_gl.asp';</script>" 
response.end()

end if
dim sqlg 
if sqlno=1 then
sqlg="update zxsp set adddate=now() where id in ("&Request("id")&")"
else
sqlg="update zxsp set adddate=getdate() where id in ("&Request("id")&")"
end if
conn.Execute ( sqlg )
if yy=2 then
call htmltvfl 
...
Response.Write "<script>window.location.href='sp_gl.asp';</script>" 
end if
call addlog("更新选中的"&shipin&"的发表时间为最新时间")
end if
...
dim sql4 
sql4="update zxsp set tj=1 where id in ("&Request("id")&")"
conn.Execute ( sql4 )
if yy=2 then
call htmltvfl 
call htmltv
...
dim sql2 
sql2="update zxsp set tj=0 where id in ("&Request("id")&")"
conn.Execute ( sql2 )
if yy=2 then
call htmltvfl 
call htmltv
...
sql = "select * from zxsp where id ="&ArticleID&""
...
call DeleteFile("../html/"&tvName&""&Separated&""&pone&""&Separated&""&ArticleID&""&Separated&"1.html")
else
arrContent=split(rs("content"),"[zmcmsfy]")
pages=Ubound(arrContent)+1
for j=1 to pages
call DeleteFile("../html/"&tvName&""&Separated&""&pone&""&Separated&""&ArticleID&""&Separated&""&j&".html")
next
end if
...
'批量删除图片
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from zxsp where ID in ("&ArticleID&")",conn,0,1
...
conn.Execute ( "delete from zxsp where id in ("&Request("id")&")" )
if yy=2 then
call htmltvfl 
call htmltv
call htmll("","","default"&lang&".html","default.asp","m=",mde,"language=",lang,"","","","","","","","")
...
call addlog("删除"&shipin&"")
response.Write "<script language=javascript>alert(""删除成功！"");window.location.href=""sp_gl.asp""</script>"
end if
%>
...
function ConfirmDel()
{
   if(confirm("确定要删除选中的<%=shipin%>吗？一旦删除将不能恢复！"))
     return true;
   else
     return false;
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center"><%=shipin%>列表管理  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="shipin.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="sp_add.asp" target="main">添加<%=shipin%></a></div></td>
...
<table width="99%" align="center" cellpadding="0" cellspacing="0"  class="dtable1">
          ...
          <tr   >
            ...
            <td width="60%"><select name="ClassID" id="ClassID" onChange="javascript:window.open(this.options[this.selectedIndex].value,'main')">
    <%
   Dim Sqlp,Rsp,TempStr
   Sqlp ="select * from spclass where ParentClassID=0 and lang='"&lang&"' order by px asc"   
   ...
   Else
	   Response.Write("<option value=""sp_gl.asp"">所有分类信息</option>") 
      Do while not Rsp.Eof   
         ...
         Response.Write(">|-" & Rsp("ClassName") & " </option>")  & VbCrLf
		    ...
		    Sqlpp ="select * from spclass Where ParentClassID="&Rsp("ID")&" and lang='"&lang&"'  order by px asc"   
			...
			Do while not Rspp.Eof 
				...
				Response.Write("</option>" )   & VbCrLf
								    ...
								    Sqlppp ="select * from spclass Where ParentClassID="&Rspp("ID")&" and lang='"&lang&"'  order by px asc"   
   ...
   End if
	...
	%>
        ...
        <form id="form1" name="form1" method="post" action="sp_gl.asp?Result=ok">
            <td width="8%" >
             快速搜索：</td>
			 <td width="22%">
...
<%
if Result="ok"   then
 DB_Sql = "select * from zxsp where title like '%"&keytag&"%' and lang='"&lang&"' "
else
if pone="" then
DB_Sql = "select * from zxsp where lang='"&lang&"' "
end if
if pone<>""  then
DB_Sql = "select * from zxsp where zmone="&Pone&" and lang='"&lang&"' "
end if
if ptwo<>""   then
DB_Sql = "select * from zxsp where  zmtwo="&Ptwo&" and lang='"&lang&"' "
end if
if pthree<>""   then
DB_Sql = "select * from zxsp where  zmthree="&Pthree&" and lang='"&lang&"' "
end if
end if
...
if Result="ok"   then
Obj_Page.Setpage_Pathurl = "sp_gl.asp?Result=ok" '执行分页的页面
else

if pone="" then
Obj_Page.Setpage_Pathurl = "sp_gl.asp" '执行分页的页面
end if
if pone<>"" then
Obj_Page.Setpage_Pathurl = "sp_gl.asp?pone="&pone&"" '执行分页的页面
end if

if ptwo<>"" then
Obj_Page.Setpage_Pathurl = "sp_gl.asp?ptwo="&ptwo&"" '执行分页的页面
end if

if pthree<>"" then
Obj_Page.Setpage_Pathurl = "sp_gl.asp?pthree="&pthree&"" '执行分页的页面
end if
...
<table width="99%"    align="center" cellpadding="0" cellspacing="0" class="dtable2">
  <tr class="wz">
    ...
    <td  ><div align="center"><%=shipin%>名称</div></td>
...
%>
  <tr >
    ...
    <td width="9%" align="center"  ><%if Rs("SmallImage")<>"" then%><img src="../<%=Rs("SmallImage")%>" width="50" height="50"><%else%><img src="../images/demo.jpg" width="50" height="50"><%end if%></td>
    <td width="12%"  ><div align="center">
      <%dim rsc
		  set rsc=server.CreateObject("adodb.recordset")
		  rsc.open "select * from spclass where lang='"&lang&"'",conn,1,1
		  while not rsc.eof
...
%></td>
    ...
    <td width="14%"  ><div align="center"><%= rs("AddDate") %></div></td>
    <td width="18%" ><div align="center"><a href="zxspxg.asp?id=<%=rs("id")%>">修改</a> <a href="spdel.asp?id=<%=rs("id")%>" onClick="return ConfirmDel();">删除</a> </div></td>
  </tr>
  <%
rs.MoveNext
Next
...
%>
  <tr >
	...
	<input class="bnt" type="submit" name="button" id="button" value="取消推荐" onClick="return confirm('确定要取消推荐吗？');" />
      ...
      <input class="bnt" type="submit" name="button" id="button" value="更新时间" onClick="return confirm('确定要更新选中的<%=shipin%>为最新时间吗？');" />
      <input class="bnt" type="submit" name="button" id="button" value="批量删除"   onclick="return confirm('确定要批量删除吗，将不可恢复?');"/>
      &nbsp; </td>
  </tr>
</table>
<form name="zmqy" method="Post">
