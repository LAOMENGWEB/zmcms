...
sql="select * from newsclass Where  mulu= '" & mulu & "'"
...
ChildCount=conn.execute("select count(*) from [newsclass] where ParentClassID="&id)(0)
conn.execute("update [newsclass] set c_child = "& checknum(ChildCount,0) &" where id="& id &"")
...
call addlog("添加"&xinwen&"分类")
response.Write "<script language=javascript>alert(""分类添加成功"");window.location.href=""fl_news.asp""</script>"
response.End
end if
%>
  ...
  <table width="99%"  border="0" cellpadding="0" cellspacing="0"  class="dtable4" align="center">
    ...
    <tr>
	 ...
	 <tr>
       ...
       * 生成html存放的路径  (最好以栏目名称的拼音缩写或者全拼 来命名。)</div></td>
    </tr>
	      <tr>
      <td>高亮值:</td><td>
