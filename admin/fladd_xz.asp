...
if request("Submit")="提交" then
	...
	sql="select * from xzclass Where mulu= '" & mulu & "'"
...
rs("ParentClassID")=ID
	...
	ChildCount=conn.execute("select count(*) from [xzclass] where ParentClassID="&id)(0)
conn.execute("update [xzclass] set c_child = "& checknum(ChildCount,0) &" where id="& id &"")
...
call htmll("","","default"&lang&".html","default.asp","m=",mde,"language=",lang,"","","","","","","","")
	...
	call addlog("添加"&xiazai&"分类")
	response.Write "<script language=javascript>alert(""分类添加成功"");window.location.href=""fl_xz.asp""</script>"
    response.End
end if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=xiazai%>分类  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
 ...
 <td ><div class="dmeun"><a href="xiazai.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="fl_xz.asp" target="main"><%=xiazai%>分类管理</a></div></td>
  ...
  <table width="99%"  border="0" cellpadding="0" cellspacing="0"  align="center" class="dtable4">
    <tr  >
		  ...
		  rsc.open "select * from xzclass where id="& request.QueryString("id")&"",conn,1,1
