...
Set rs = server.CreateObject("adodb.recordset")
rs.open "Select * From download ",conn,1,3
...
'获取添加新闻的ID  生成html开始
set rs=conn.execute("select top 1 id,zmone,zmtwo,zmthree from [Download] order by id desc")
xzid=rs(0)
pone=rs(1)
ptwo=rs(2)
pthree=rs(3)
...
call htmll("/html/","",""&downname&""&Separated&""&pone&""&Separated&""&xzid&""&Separated&"1.html",""&showdown&"/index.asp","pone=",pone,"downID=",xzid,"m=",m,"language=",lang,"page=",1,"","")
else
arrContent=split(content,"[zmcmsfy]")
pages=Ubound(arrContent)+1
for i=1 to pages
call htmll("/html/","",""&downname&""&Separated&""&pone&""&Separated&""&xzid&""&Separated&""&i&".html",""&showdown&"/index.asp","pone=",pone,"downID=",xzid,"m=",m,"language=",lang,"page=",i,"","")
next
end If
...
if Instr(content,"[zmcmsfy]")=0 then
call htmll("/html/","",""&downname&""&Separated&""&pone&""&Separated&""&xzid&""&Separated&"1.html",""&showdown&"/index.asp","pone=",pone,"downID=",ID,"m=",m,"language=",lang,"page=",1,"","")
else
arrContent=split(content,"[zmcmsfy]")
pages=Ubound(arrContent)+1
for i=1 to pages
call htmll("/html/","",""&downname&""&Separated&""&pone&""&Separated&""&xzid&""&Separated&""&i&".html",""&showdown&"/index.asp","pone=",pone,"downID=",ID,"m=",m,"language=",lang,"page=",i,"","")
next
end If
...
call addlog("添加"&xiazai&"")
response.Write "<script language=javascript>alert(""恭喜您添加成功"");window.location.href=""Down_Manage.asp""</script>"
end if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=xiazai%>  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table width="99%" border="0" cellpadding="0" cellspacing="0" align="center" class="dtable4">
          ...
          <tr> 
            <td width="14%" ><%=xiazai%>名称：</td>
           <td width="86%" > <input name="Title" type="text" class="input" id="Title" size="30" datatype="*" nullmsg="名称不能为空">  <font color="#FF0000">*</font>  
			</td>
 ...
 <input   type="button" id="colorpicker" value="打开取色器" /></td>
  ...
  <td >tags:</td>
     ...
     【<a href="#" id="KeyLinkByTitle" style="color:green">根据标题自动获取</a>】(只对中文有效，多个tags用|隔开)</br></br>
     </td>
...
<td><%=xiazai%>类别：</td>
	   ...
	   <%call mxlb("xzClass",ClassId)%>
          ...
          <tr>
           ...
           <td >
              <label>
              <input name="downkey" type="text" id="downkey" size="100" />
            </label></td>
          </tr>
          <tr>
            <td ><%=xiazai%>描述：</td>
           <td >
              <label>
              <input name="downms" type="text" id="downms" size="100" />
            </label></td>
 ...
 afterBlur: function(){this.sync();}
 	...
 	K('input[name=appendHtml]').click(function(e) {
          ...
          <tr> 
            <td ><%=xiazai%>说明：</td>
           <td >
            <textarea name="Content" id="Content" style="width:700px;height:300px;visibility:hidden;" datatype="*" nullmsg="内容不能为空"></textarea> </td>
          </tr>
		   <tr >
		     <td >手动分页：</td>
...
幻灯图片地址：<input type="text" id="IcoImage" name="IcoImage" value="" style="width:600px;" /><br/><br/>
	                  <input name="button" type="button" id="J_selectImage" style=" background:url(images/allimg.png) no-repeat; width:170px;height:34px;border:none" value="" /><br/><br/>
                   多图片地址： <input type="text" id="ImagePath" name="ImagePath" value="" style="width:600px;" /> 
          ...
          <tr> 
            <td  >适用系统：</td>
           <td > <input name="System" type="text"  id="System"  value="所有系统" size="42">
            (例如：Win98/Win2000/Win2003/WinXP/vista/win7)</td>
          </tr>
          <tr>
            <td >软件语言：</td>
           <td >
            <input name="Language" type="text"  id="Language" value="简体中文" size="30"></td>
          </tr>
          <tr> 
            <td  >软件类型：</td>
           <td > <input name="Softclass" type="text"  id="Softclass"  value="软件" size="30">
            </td>
          </tr>
		
          <tr > 
            <td  ><%=xiazai%>地址一：</td>
           <td ><input name="DownloadUrl" type="text" id="url" value="#" size="50" /> 
            <input type="button" id="insertfile" value="选择文件" /></td>
          </tr>
		   <tr > 
            <td  ><%=xiazai%>地址二：</td>
           <td ><input name="DownloadUrl1" type="text" id="url11" value="#" size="50" /> 
            </td>
          </tr>
		   <tr > 
            <td  ><%=xiazai%>地址三：</td>
           <td ><input name="DownloadUrl2" type="text" id="url22" value="#" size="50" /> 
           </td>
          </tr>
        ...
        <td >查看权限：</td>
          ...
          <tr>
            <td  >文件大小：</td>
           <td ><input name="zdy1" type="text" id="zdy1" size="10" />
            <label>(例如：10M 105KB )</label></td>
          </tr>
		   <tr >
                     <td  >是否推荐：</td>
          ...
          <tr> 
           ...
           <td >
                ...
                <input name="AddDate" type="text" id="AddDate" value="<%=now()%>" onClick="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm:ss'})" >  提示：【可以定时发布<%=xiazai%>了，点击边框设定时间】            </td>
          </tr>
          <tr> 
            <td  colspan="2"> <input type="submit" name="Submit" value="提交" class="bnt">
              　            </td>
          </tr>
</table>
</form>
