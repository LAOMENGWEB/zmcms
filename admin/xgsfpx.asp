...
sql1="select * from province where id="&request.QueryString("id")&""
...
rs.close
set rs=nothing
call addlog("设置营销区域排序")
end if
Set rs = server.CreateObject("adodb.recordset")
sql2 = "select * from province where id = "&request.QueryString("id")&""
...
End If
%>
<form id="form1" name="form1" method="post" action="xgsfpx.asp?id=<%=rs("id")%>">
  <label>
