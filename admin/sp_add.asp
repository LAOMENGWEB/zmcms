...
rs("spheight")=spheight
	rs("spurl") = request.Form("spurl")
	rs("wapspurl") = request.Form("wapspurl")
	rs("arrspurl") = request.Form("arrspurl")
	if ParentClassID=0 then
...
'获取添加新闻的ID  生成html开始
set rs=conn.execute("select top 1 id,zmone,zmtwo,zmthree from [zxsp] order by id desc")
tvid=rs(0)
pone=rs(1)
ptwo=rs(2)
pthree=rs(3)
...
call htmll("/html/","",""&tvname&""&Separated&""&pone&""&Separated&""&tvid&""&Separated&"1.html",""&showvideo&"/index.asp","pone=",pone,"videoID=",tvid,"m=",m,"language=",lang,"page=",1,"","")
else
arrContent=split(content,"[zmcmsfy]")
pages=Ubound(arrContent)+1
for i=1 to pages
call htmll("/html/","",""&tvname&""&Separated&""&pone&""&Separated&""&tvid&""&Separated&""&i&".html",""&showvideo&"/index.asp","pone=",pone,"videoID=",tvid,"m=",m,"language=",lang,"page=",i,"","")
next
end If
...
for i=1 to totalpage
call htmll("/"&rs1("mulu")&"/"&rs2("mulu")&"/"&rs3("mulu")&"/","",""&tvclass&""&Separated&""&pthree&""&Separated&""&i&".html",""&video&"/index.asp","pone=",pone,"ptwo=",ptwo,"pthree=",pthree,"m=",m,"language=",lang,"Page=",i)
next
End If
...
call addlog("添加"&shipin&"")
response.Write "<script language=javascript>alert(""添加成功！"");window.location.href=""sp_gl.asp""</script>"
response.End
End If
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">增加<%=shipin%>  <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 ...
 <div class="dmeun"><a href="sp_gl.asp" target="main"><%=shipin%>管理</a></div>
                ...
                <table width="99%"  border="0" cellpadding="0" cellspacing="0" align="center" class="dtable4">
                  ...
                  <tr> 
                    <td width="14%" > <%=shipin%>名称：</td>
                    <td width="86%"><input name="Title" type="text" id="Title" value="" size="40" datatype="*" nullmsg="名称不能为空"/>  <font color="#FF0000">*</font></td>
					
				  </tr>
					 <tr> 
                    <td > 标题颜色：</td>
					<td>
					<input name="btcolor"type="text" id="color"   value="" size="8" /> 
       ...
       <input name="KeyWord" type="text" id="KeyWord" size="40"  datatype="*" nullmsg="tags不能为空"> <font color="#FF0000">*</font>
  【<a href="#" id="KeyLinkByTitle" style="color:green">根据标题自动获取</a>】(只对中文内容生效，多个tags用|隔开)</br></br>
     </td>
...
<select ID="ClassID" name="ClassID" datatype="*" nullmsg="所属分类不能为空">
	  ...
	  </select> <font color="#FF0000">*</font>（有三级栏目的必须选择三级栏目）</td>
</tr>
				  <tr>
				    <td><%=shipin%>关键词：</td>
					<td>
			        <input name="tvkey" type="text" id="tvkey" size="100" /></td>
			      </tr>
				  <tr>
				    <td><%=shipin%>描述：</td>
					<td>
			        <input name="tvms" type="text" id="tvms" size="100" /></td>
			      </tr>
...
<%call showColumn("zxsp",5,id)%>
        ...
        <td >查看权限：</td>
          ...
          专属（隶属：权限值≥可查看，专属：权限值＝可查看）</td>
			  ...
			  <tr>
		        ...
		        <div id="mysmaimg"></div>		        </td>
				  ...
				  <tr>
				    <td >单<%=shipin%>播放地址：</td>
					<td><input name="spurl" type="text" id="url" value="" style="width:400px" /> 
		  ...
		  支持调用各大视频网站的swf 地址（例子：http://www.aipai.com/c13/NDY5ISYnJGgnaiQg/playerOut.swf)</td>
				  	  ...
				  	  <tr>
				    <td>wap<%=shipin%>播放地址：</td>
					<td><input name="wapspurl" type="text" id="url1" value="" style="width:400px" /> 
            <input type="button" id="insertfile1" value="选择文件" /> (手机只支持MP4格式的视频)</td>
			      </tr>
			    ...
			    <tr>
					...
					<td>
				        ...
				        <textarea name="arrspurl" rows="5" id="arrspurl" style="width:400px"></textarea>
			          多文件连播地址：只支持flv,mp4, 多个文件请用|隔开.可以是flv或者mp4， 如果调用多文件连播，请把 单视频播放地址清空，同样如果只调用单文件播放，请把多文件连播清空。  只支持调用flv,mp4的格式。</br>					  </td>
			      </tr>	  
 	...
 	K('input[name=appendHtml]').click(function(e) {
			...
			prettyPrint();
			        ...
			        <tr>
			          <td >播放屏幕的背景颜色:</td>
			           <td>
			            <input type="text" name="screencolor"  value="000000"/>   颜色不带#号 加入黑色#000000 只需输入#号后面的6位就可以了
		             </td>
		          </tr>
			        <tr>
			          <td >控制面板放置位置:</td>
			          ...
			          <td >播放器封面图片延展方式:</td>
			           <td>
			            ...
			            <select name="stretching" id="stretching">
		                  <option value="none">不延展</option>
		                  <option value="exactfit">不锁定宽高填充</option>
		                  <option value="fill" >锁定宽高填充</option>
		                  <option value="uniform" selected="selected">锁定以黑色填充空白</option>
	                    </select>
		              </td>
		          </tr>
		            <tr>
		              <td >播放器皮肤:</td>
			           <td>
		                ...
		                <input type="text" name="skin" value="55"/>
	                  填写 数字 1-93 </td>
	              </tr>
				   <tr>
		              <td >播放器宽度:</td>
			           <td>
		                ...
		                <input type="text" name="spwidth"  value="400"/>
	                  </td>
	              </tr>
				   <tr>
		              <td>播放器高度:</td>
			           <td>
		                <input type="text" name="spheight" value="300"/>
	                  </td>
	              </tr>
	              <tr> 
				  <td ><%=shipin%>说明：</td>	<td> <textarea datatype="*" nullmsg="<%=shipin%>内容不能为空" name="Content" id="Content" style="width:700px;height:300px;visibility:hidden;"></textarea></td>
                  </tr>
 ...
 <div id="J_imageView">  </div></td>
                  ...
                  <tr>
                    <td >是否推荐 ：</td><td> <input name="tj" type="checkbox" id="tj" value="1"></td>
                  </tr> 
					  <tr > 
            <td  >点击次数：</td>
			<td>
              <input  name="hits" type="text" class="input" value="0"  size="30"> 
			  </td>
          </tr>
				  <tr > 
            <td  >发布人：</td><td>
          ...
          <tr > 
            ...
            <input name="ly"type="text" id="ly"   value="本站"></td>
                  ...
                  <tr>
                    <td >发布时间：</td><td><input name="adddate" type="text" id="adddate" value="<%=now()%>" onClick="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm:ss'})" />[可以定时发布视频了，点击边框设定时间]</td>
                  </tr>
                  <tr> 
                    <td colspan="2">  
