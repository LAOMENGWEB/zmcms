...
rs.open "Select * From dy ",conn,1,3
rs.addnew
...
'获取添加单页的ID  生成html开始
set rs=conn.execute("select top 1 id from [dy] order by id desc")
dyid=rs(0)
rs.close:set rs=Nothing

if yy=2 then
sql="Select * from dy where id="&dyid&" order by ID desc"
...
call htmll("/html/","",""&aboutname&""&Separated&""&pone&""&Separated&""&dyid&""&Separated&"1.html",""&aabout&"/index.asp","pone=",pone,"aboutID=",dyid,"m=",m,"language=",lang,"page=",1,"","")
else
arrContent=split(content,"[zmcmsfy]")
pages=Ubound(arrContent)+1
for i=1 to pages
call htmll("/html/","",""&aboutname&""&Separated&""&pone&""&Separated&""&dyid&""&Separated&""&i&".html",""&aabout&"/index.asp","pone=",pone,"aboutID=",dyid,"m=",m,"language=",lang,"page=",i,"","")
next
end if
...
call addlog("添加"&danye&"信息")
response.Write "<script language=javascript>alert(""恭喜您添加成功"");window.location.href=""dy.asp""</script>"
end if
%>
...
<tr>
 <td  style="border-bottom:none;border-right:none;color:#cccccc"><div align="center">添加<%=danye%> <span id="yuyan" style="margin-left:20px"></span></div></td>
</tr>
</table>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 ...
 <td ><div class="dmeun"><a href="danye.asp" target="main">返回菜单</a></div><div class="dmeun"><a href="dy.asp" target="main"><%=danye%>列表管理</a></div><div class="dmeun"><a href="fl_dy.asp" target="main"><%=danye%>分类管理</a></div></td>
   ...
   <table width="99%"  border="0" cellpadding="0" cellspacing="0"  align="center" class="dtable4">
	 ...
	 <tr  > 
                    <td width="12%"  > <%=danye%>名称:  </td>
       <td width="18%"  ><input type="text" name="Title" class="inputtext"  size="25" datatype="*" nullmsg="<%=danye%>名称不能为空"></td>
                    <td width="70%"  > <div class="Validform_checktip"><%=danye%>名称不能为空</div> </td>
	 </tr> 
    ...
    Else
		...
		set rsc=nothing
		  %>
				  ...
				  <tr  >
				    <td  >外链地址:			        </td>
			        <td ><input name="wlurl" type="text" id="wlurl" /></td>
					<td>如果不是外链地址请为空</td>
				  </tr>
				  <tr  > 
                    <td  > <%=danye%>关键字: </td>
                    <td  colspan="2"><input name="gjz"type="text" class="inputtext" id="gjz"   size="80" ></td>
				  </tr>
				  ...
				  <tr  > 
					...
					<td colspan="2"  ><input name="ms" type="text" class="inputtext" id="ms"  size="80"  /></td>
                  </tr>
				  	<%'自定义字段%>
        ...
        <td><input name="Exclusive" class="none" type="radio" value="&gt;="  <%if Exclusive="" or Exclusive=">=" then response.write ("checked")%>> 
          隶属
           <input type="radio" class="none"  <%if Exclusive="=" then response.write ("checked")%> name="Exclusive" value="=">
	   ...
	   <tr >
                ...
                <td colspan="2">  <input name="SmallImage"  type="text" id="weepic"  value="" size="50"  /> <br/> <br/>  	 <a class='sfile' href="Upload_All.Asp?add=one" title="支持上传格式(*.swf;*.gif;*.bmp)"><img src="Images/dtup.jpg"  border="0"/></a>
 ...
 afterBlur: function(){this.sync();}
 	...
 	K('input[name=appendHtml]').click(function(e) {
			...
			prettyPrint();
				  ...
				  <tr  > 
                    <td> <%=danye%>内容: </br></td>
                    <td  colspan="2"><textarea datatype="*" nullmsg="<%=danye%>内容不能为空" name="Content" id="Content" style="width:700px;height:300px;visibility:hidden;" > </textarea></td>
				  </tr>
 ...
 幻灯图片地址：<input type="text" id="IcoImage" name="IcoImage" value="" style="width:600px;" /><br/></br>
	         ...
	         <td  colspan="2"><INPUT type=button class="bnt" name="appendHtml" value="插入分页符"></td>
                 ...
                 <tr  > 
                    <td  > 排序：</td>
                    <td  colspan="2"><input name="px" type="text" class="inputtext" id="px"   size="4"   value="1"/></td>
     </tr>
                  <tr  > 
