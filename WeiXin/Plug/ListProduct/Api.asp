...
dim Rs,Sql
dim Rs2,Sql2
dim tablename:tablename="product"		'''''''''''''''''''''''''''
dim plugpath:plugpath="ListProduct"		'''''''''''''''''''''''''''
...
if k_plugParam<>0 then
	...
	If not Rs2.eof then
		...
		If not Rs.eof then
			...
			Do While Not Rs.Eof
				Title=rs("Title")
				Descriptions=CutStrX(nohtml1(nohtml(rs("Content"))),200)
                ...
                rszm.open "select * from cpclass where id="&rs("zmone")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				Url=weburl&picroot&"wap/showproduct/?pone="&rs("zmone")&"&proid="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				if rs("zmone")<>0 and rs("zmtwo")<>0 and rs("zmthree")=0 then
				 set rszm=server.CreateObject("adodb.recordset")
                rszm.open "select * from cpclass where id="&rs("zmtwo")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				Url=weburl&picroot&"wap/showproduct/?pone="&rs("zmone")&"&ptwo="&rs("zmtwo")&"&proid="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				if rs("zmone")<>0 and rs("zmtwo")<>0 and rs("zmthree")<>0 then
				 set rszm=server.CreateObject("adodb.recordset")
                rszm.open "select * from cpclass where id="&rs("zmthree")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				Url=weburl&picroot&"wap/showproduct/?pone="&rs("zmone")&"&ptwo="&rs("zmtwo")&"&pthree="&rs("zmthree")&"&proid="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				else
