...
<%
	...
	sub adddb()
		...
		if Cls.db.dbnew("[Plug_PicMsgList]",data,"")=1 then
			Cls.echo "1"
		else
			Cls.echo "0数据出错"
		end if	
	...
	sub editdb()
		...
		if Cls.db.dbupdate("[Plug_PicMsgList]","p_Id="&id,data)=1 then
			Cls.echo "1"
		else
			Cls.echo "0数据出错"
		end if	
	...
	sub del()
		dim id:id=Cls.getint(Cls.fget("id",0),0)
		if id>0 then
			Cls.db.dbdel "[Plug_PicMsgList]","p_id="&id&""			
			Cls.db.dbdel "[sys_KeyWord]","k_plugName='picMsgList' and k_plugParam="&id&""	'删除关键字已指定的素材
		end if
		Cls.echo "1"
		Cls.die
	end sub
		...
		if ubound(idarr)<0 then
			...
			for i=0 to ubound(idarr)
				if not isnumeric(idarr(i)) then
					...
					Cls.db.dbdel "[sys_KeyWord]","k_plugName='picMsgList' and k_plugParam="&id&""	'删除多图文素材关联的关键字
				end if
			next	
			Cls.db.dbdel "[Plug_PicMsgList]","p_id in("&id&")"
			Cls.echo "1"
		end if
		Cls.die
	end sub
...
{
	var picMsgList=document.getElementById('picMsgList');
	var ItemList
	ItemList="";
	for(var i=0;i<picMsgList.options.length;i++){
		 ItemList +=picMsgList.options[i].value + ",";
	}
	...
	if (picMsgList.options.length<1){
		$.message({content:"请选择图文素材！"});
		return false;
		
	}
	if (picMsgList.options.length>10){
...
function removeItem(){
   ...
   for(var i=0;i<DataList.options.length;i++)
   {
	   ...
	   if(tempOption.selected){
		   ...
		   picMsgList.appendChild(tempOption);
	   }    
   }
}
...
function addItem(){
   ...
   for(var i=0;i<picMsgList.options.length;i++)
   {
	   var tempOption=picMsgList.options[i];
	   if(tempOption.selected){
		   picMsgList.removeChild(tempOption);
...
function getdata(t0,t1,t2){
	...
	if(!t0==""){
		$.ajax({
			   type: "post",
			   url:"<%=webroot%>plug/"+plug+"/Admin_Api.asp?act=getlist&id="+id,
			   cache:false,
			   timeout:10000,
...
<body>
	...
	if act="edit" then	
	dim id:id=Cls.getint(Cls.fget("id",0),0)
	datadb=Cls.db.dbload("","p_Title,p_IdList","[Plug_PicMsgList]","p_id="&id,"")
	%>
    <script>	
	$(function(){
		getdata("picMsgList","","DataList");
		getdata("picMsgList","<%=datadb(1,0)%>","picMsgList");
	})
	</script>
...
<tr > 
<td width="10%"   >多图文集标题：</td>
<td width="88%">
<input type="text" name="p_Title" size="50" value="<%=datadb(0,0)%>"/></td>
 </tr>	
...
<td width="88%">
     ...
     <select ondblclick="addItem();" id="picMsgList"  multiple="true" style="height:150px;width:300px">
...
<td width="10%"   >多图文集标题：</td>
<td width="88%">
<input type="text" name="p_Title" size="50" /></td>
 </tr>	
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td >
 <div class="dmeun"><a href="?act=add" target="main" style="color:#ffffff">添加多图文素材</a></div>
...
<table width="99%"   cellpadding="0" cellspacing="0"  class="dtable2" align="center">
      <tr class="wz">
		...
		dim SqlStr:SqlStr="Select p_Id,p_Title,p_idList,p_Addtime from [Plug_PicMsgList] order by p_Id desc"
		Page.Sql = SqlStr
		Page.PageSize = 20
		Page.Style = 5
		Set Rs = Page.Rs
