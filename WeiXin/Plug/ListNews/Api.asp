...
dim plugpath:plugpath="ListNews"		'''''''''''''''''''''''''''
	...
	if sqlno=1 then
		...
		If not Rs.eof then
			...
			Do While Not Rs.Eof
                ...
                rszm.open "select * from newsclass where id="&rs("zmone")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				
				Url=weburl&picroot&"wap/showinfo/?pone="&rs("zmone")&"&newsID="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				if rs("zmone")<>0 and rs("zmtwo")<>0 and rs("zmthree")=0 then
					 set rszm=server.CreateObject("adodb.recordset")
                rszm.open "select * from newsclass where id="&rs("zmtwo")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				Url=weburl&picroot&"wap/showinfo/?pone="&rs("zmone")&"&ptwo="&rs("zmtwo")&"&newsID="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				if rs("zmone")<>0 and rs("zmtwo")<>0 and rs("zmthree")<>0 then
					 set rszm=server.CreateObject("adodb.recordset")
                rszm.open "select * from newsclass where id="&rs("zmthree")&" ",conn,1,1
                mm=rszm("m")
                rszm.close
                Set rszm=Nothing
				Url=weburl&picroot&"wap/showinfo/?pone="&rs("zmone")&"&ptwo="&rs("zmtwo")&"&pthree="&rs("zmthree")&"&newsID="&rs("id")&"&m="&mm&"&language="&rs("lang")
				end if
				else
