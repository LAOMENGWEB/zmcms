...
<%
	...
	sub adddb()
		...
		if Cls.db.dbnew("[Plug_PicMsg_class]",data,"c_ClassName='"&c_ClassName&"'")=1 then
			Cls.echo "1"
		else
			Cls.echo "0该分类已存在"
		end if	
	...
	sub editdb()
		...
		Dim c_ClassName:c_ClassName=Cls.enhtml(Cls.fpost("c_ClassName",0))		
		Dim data:data=array(array("c_ClassName",c_ClassName,50,1))
		if Cls.db.dbupdate("[Plug_PicMsg_class]","c_id="&id,data)=1 then
			Cls.echo "1"
		else
			Cls.echo "0数据出错"
		end if	
	...
	sub del()
		dim id:id=Cls.getint(Cls.fget("id",0),0)
		if id>0 then
			Cls.db.dbdel "[Plug_PicMsg_class]","c_id="&id&""
		end if
		Cls.echo "1"
		Cls.die
	end sub
...
<body>
	...
	if act="edit" then	
	dim id:id=Cls.getint(Cls.fget("id",0),0)
	datadb=Cls.db.dbload("","c_ClassName","[Plug_PicMsg_class]","c_id="&id,"")
	%>
...
<div class="dmeun"> <a href="admin_class.asp" target="main" style="color:#ffffff">图文分类管理</a></div>
 </td>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 ...
 <div class="dmeun"><a href="?act=add" target="main" style="color:#ffffff">添加分类</a></div>
 <div class="dmeun"><a href="Admin.asp" target="main" style="color:#ffffff">图文素材管理</a></div>
  ...
  <table width="99%"   cellpadding="0" cellspacing="0"  class="dtable2" align="center">
      <tr class="wz">
        ...
        <td>管理</td>
      	...
      	<%datadb=Cls.db.dbload("","c_id,c_ClassName","[Plug_PicMsg_class]","","c_id asc")
