...
rs.close
set rs=nothing
response.Write "<script language=javascript>alert(""恭喜您修改成功"");window.location.href=""admin.asp""</script>"
end if
...
id=request.querystring("id")
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open "Select * From Plug_PicMsg where p_id="&id, conn2,1,1
...
<td width="10%"   >图文标题：</td>
<td width="88%">
 <input type="text" name="p_Title" size="50" datatype="*" nullmsg="图文标题不能为空" value="<%=rs("p_Title")%>"/>
</td>
 </tr>
 <tr > 
<td width="10%"   >所属分类：</td>
  ...
  <%
    ...
    Else
		  while not rsc.eof
			...
			response.Write("<option value=""" & rsc("c_id") & """ selected>" & rsc("c_classname") & "</option>")
			else
...
<td width="10%"   >描述：</td>
<td width="88%">
<textarea name="p_Info" cols="60" rows="5" datatype="*" nullmsg="描述不能为空" value=""><%=rs("p_Info")%></textarea>
</td>
 </tr>
...
<select name="p_Type" id="p_Type" >
          <option value="图文" <%if rs("p_Type")="图文" then Response.Write("selected")%>>图文</option>
          <option value="链接" <%if rs("p_Type")="链接" then Response.Write("selected")%>>链接</option>
        </select>
	...
	<td>
        ...
        <input type="text" name="p_Url" id="p_Url" size="60" value="<%=rs("p_Url")%>"/>  当素材类型为 链接时  此性内容必须填写
		</td>
        </tr>
		    <td >缩略图：
				   <input name="p_Pic"  type="hidden" id="weepic"  value="<%=rs("p_Pic")%>" size="50"  />   		        </td>
                <td colspan="2"> <a class='sfile' href="Upload_All.Asp?add=one" title="支持上传格式(*.swf;*.gif;*.bmp)"><img src="dtup.jpg"  border="0"/></a>
		        <br />
<div id="mysmaimg"><%if rs("p_Pic")<>"" then%>
<img src="../../../<%=rs("p_Pic")%>" id="sc"/><br />
<a href="delslt.asp?sltdz=<%=rs("p_Pic")%>" target="_blank" onclick="deltp()" id="lj">删除</a>
<%end if%>
</div></td>
	      </tr>
   ...
   <tr > 
                    <td> 图文内容: </br></td>
                    <td  colspan="2"><textarea  name="p_Content" id="p_Content" style="width:700px;height:300px;visibility:hidden;" datatype="*" nullmsg="图片内容不能为空"><%=rs("p_Content")%> </textarea>
...
<a href="javascript:setContent('<%=img(i)%>')" ><img title="点击图片将图片插入到编辑器" src="<%=img(i)%>" border="0" /></a><br> <a href="javascript:dropThisDiv('<%=ids%>','<%=img(i)%>')"">删除</a></div>
 <%
 next
 end if
 %>      </div></td>
 ...
 <tr > 
            <td colspan="2"> <input class="bnt" name="submit" type="submit" value="修改">
