...
<%
	...
	sub adddb()
		...
		if Cls.db.dbnew("[Plug_ListGoods]",data,"")=1 then
			Cls.echo "1"
		else
			Cls.echo "0数据出错"
		end if	
	...
	sub editdb()
		...
		if Cls.db.dbupdate("[Plug_ListGoods]","p_Id="&id,data)=1 then
			Cls.echo "1"
		else
			Cls.echo "0数据出错"
		end if	
	...
	sub del()
		dim id:id=Cls.getint(Cls.fget("id",0),0)
		if id>0 then
			Cls.db.dbdel "[Plug_ListGoods]","p_id="&id&""			
			Cls.db.dbdel "[sys_KeyWord]","k_plugName='ListGoods' and k_plugParam="&id&""	'删除关键字已指定的素材
		end if
		Cls.echo "1"
		Cls.die
	end sub
		...
		if ubound(idarr)<0 then
			...
			for i=0 to ubound(idarr)
				if not isnumeric(idarr(i)) then
					...
					Cls.db.dbdel "[sys_KeyWord]","k_plugName='ListGoods' and k_plugParam="&id&""	'删除多图文素材关联的关键字
				end if
			next	
			Cls.db.dbdel "[Plug_ListGoods]","p_id in("&id&")"
			Cls.echo "1"
		end if
		Cls.die
	end sub
...
{
	var ListGoods=document.getElementById('ListGoods');
	var ItemList
	ItemList="";
	for(var i=0;i<ListGoods.options.length;i++){
		 ItemList +=ListGoods.options[i].value + ",";
	}
	...
	if (ListGoods.options.length<1){
		$.message({content:"请选择图文素材！"});
		return false;
		
	}
	if (ListGoods.options.length>10){
...
function removeItem(){
   ...
   for(var i=0;i<DataList.options.length;i++)
   {
	   ...
	   if(tempOption.selected){
		   ...
		   ListGoods.appendChild(tempOption);
	   }    
   }
}
...
function addItem(){
   ...
   for(var i=0;i<ListGoods.options.length;i++)
   {
	   var tempOption=ListGoods.options[i];
	   if(tempOption.selected){
		   ListGoods.removeChild(tempOption);
...
<body>
	...
	if act="edit" then	
	dim id:id=Cls.getint(Cls.fget("id",0),0)
	datadb=Cls.db.dbload("","p_Title,p_IdList","[Plug_ListGoods]","p_id="&id,"")
	%>
    <script>	
	$(function(){
		getdata("ListGoods","","DataList");
		getdata("ListGoods","<%=datadb(1,0)%>","ListGoods");
	})
	</script>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td >
  <div class="dmeun"><a href="admin.asp" target="main" style="color:#ffffff">单页素材管理</a></div>
...
<td width="88%">
  <select ondblclick="removeItem();" id="DataList" multiple="true" style="height:150px;width:300px">
     ...
     <select ondblclick="addItem();" id="ListGoods"  multiple="true" style="height:150px;width:300px">
...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="99%" class="dtable3">
 <tr>
 <td ><div class="dmeun"><a href="?act=add" target="main" style="color:#ffffff">添加单页素材</a></div>
 <div class="dmeun"><a href="javascript:;" class="delsome" target="main" style="color:#ffffff">批量删除</a></div>
   ...
   <table width="99%"   cellpadding="0" cellspacing="0"  class="dtable2" align="center">
      <tr>
		...
		dim SqlStr:SqlStr="Select p_Id,p_Title,p_idList,p_Addtime from [Plug_ListGoods] order by p_Id desc"
		Page.Sql = SqlStr
		Page.PageSize = 20
		Page.Style = 5
		Set Rs = Page.Rs
